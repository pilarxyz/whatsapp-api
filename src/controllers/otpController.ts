...
export const verify = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Only proceed if message is provided in the request body
    if (data.message) {
      let formattedMessage = {
        text: data.message,
      } as any;
      ...
      // If no message provided, just send a response without sending a message
      return ResponseUtil.ok({
        res,
